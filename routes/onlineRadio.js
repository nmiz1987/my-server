...
router.get("/", async (req, res) => {
  try {
    ...
    const allStations = await stationsModel
      .find(searchParams)
...
router.get("/page/", async (req, res) => {
  try {
    ...
    const order = req.query?.order === "desc" ? -1 : 1;
    const allData = await stationsModel.find(searchParams).limit(9999999).sort({ votes: order });
...
router.get("/generateProps", async (req, res) => {
  try {
    ...
    allStations.forEach((station) => {
      ...
      if (!props.languages_codes.includes(station.languagecodes) && station.languagecodes != "" && station.languagecodes.split(",").length == 1) {
