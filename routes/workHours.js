...
router.post("/new-entrance", auth, async (req, res) => {
    try {
        ...
        logAction(
            req.body.email,
            `New work hours created: email: ${newHours.email}, date: ${newHours.date}, startTime: ${newHours.startTime}`
        );
...
async function getAllDaysByToken(req, res, next) {
    let work;
    try {
        work = await workHoursModel.find({ email: res.email });
        if (work == null) {
            return res.status(404).json({ message: `Cannot find work hours for email ${res.email}` });
        }
    } catch (err) {
...
async function getOneDayByToken(req, res, next) {
    ...
    let work;
    try {
        work = await workHoursModel.findOne({ email: res.email, date: req.body.date });
        if (work == null) {
            return res
                .status(404)
                .json({ message: `Cannot find work day for email ${res.email}, date ${req.body.date}` });
        }
    } catch (err) {
        return res.status(500).json({ message: err.message });
    }
